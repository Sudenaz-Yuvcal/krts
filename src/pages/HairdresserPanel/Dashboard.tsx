...
export const Dashboard: React.FC = () => {
  ...
  const [viewType, setViewType] = useState<"all" | "service" | "employee">(
    "all",
  );
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const appointmentsArray: DbAppointment[] =
          (dbAppointments as unknown as DbAppointment[]) || [];
        ...
        appointmentsArray.forEach((appointment) => {
          ...
          if (!serviceMap[serviceName])
            serviceMap[serviceName] = { count: 0, rev: 0 };
          ...
          if (!employeeMap[employeeName])
            employeeMap[employeeName] = { count: 0, rev: 0 };
          ...
          .map((name) => {
            ...
            return {
              ...
              percentage: Math.round(
                (serviceItem.rev / (totalAppointmentEarnings || 1)) * 100,
              ),
            };
          })
          .sort(
            (firstService, secondService) =>
              secondService.totalRevenue - firstService.totalRevenue,
          );
        setTopServices(formattedServices);

        const formattedEmployees = Object.keys(employeeMap)
          .map((name) => {
          ...
          .sort(
            (firstEmployee, secondEmployee) =>
              secondEmployee.totalRevenue - firstEmployee.totalRevenue,
          );
        ...
        const timeline = Array.from({ length: dayLimit })
          .map((_, index) => {
        ...
        appointmentsArray.forEach((appointment) => {
          ...
          const appointmentDateClean =
            appointment.appointment_date.split("T")[0];
          const matchedTimelineDay = timeline.find(
            (day) => day.dateString === appointmentDateClean,
          );

          if (matchedTimelineDay) {
            matchedTimelineDay.serviceAmount +=
    ...
    <div className="space-y-6 sm:space-y-9 w-full px-2 sm:px-0">
      <div>
        ...
        <p className="text-xs font-semibold text-slate-400 mt-1">
          Supabase canlı veritabanı bağlantılı, sadece hizmet ve çalışan odaklı
          gerçek zamanlı salon analizleri.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8">
        <div onClick={() => setViewType("all")} className="cursor-pointer">
          <Card
            className={`border p-5 sm:p-6 rounded-3xl sm:rounded-[28px] shadow-xs transition-all duration-200 ${viewType === "all" ? "border-emerald-500 bg-emerald-50/5 ring-4 ring-emerald-50" : "border-slate-100 bg-white hover:border-emerald-200"}`}
          >
            <div className="flex items-center justify-between">
              <div>
                <span className="text-[12px] sm:text-[13px] font-bold text-slate-400 tracking-tight">
                  Toplam Toplanan Ciro
                </span>
                <h3 className="text-2xl sm:text-3xl font-black text-slate-800 mt-1.5">
                  ₺
                  {totalSale.toLocaleString("tr-TR", {
        ...
        <div onClick={() => setViewType("service")} className="cursor-pointer">
          <Card
            className={`border p-5 sm:p-6 rounded-3xl sm:rounded-[28px] shadow-xs transition-all duration-200 ${viewType === "service" ? "border-indigo-500 bg-indigo-50/5 ring-4 ring-indigo-50" : "border-slate-100 bg-white hover:border-indigo-200"}`}
          >
            <div className="flex items-center justify-between">
              <div>
                <span className="text-[12px] sm:text-[13px] font-bold text-slate-400 tracking-tight">
                ...
                <span className="text-[11px] font-semibold text-indigo-600 block mt-1">
                  Hizmet Verilen İşlem
                </span>
              </div>
        ...
        >
          <Card
            className={`border p-5 sm:p-6 rounded-3xl sm:rounded-[28px] shadow-xs transition-all duration-200 ${viewType === "employee" ? "border-purple-500 bg-purple-50/5 ring-4 ring-purple-50" : "border-slate-100 bg-white hover:border-purple-200"}`}
          >
            <div className="flex items-center justify-between">
              <div>
                <span className="text-[12px] sm:text-[13px] font-bold text-slate-400 tracking-tight">
          ...
          <div className="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-slate-100">
              ...
              <div className="space-y-4 animate-fadeIn mb-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <h5 className="text-xs sm:text-sm font-black text-slate-800 uppercase tracking-wider flex items-center gap-2">
                    <Scissors className="w-4 h-4 text-indigo-500" /> Hizmet
                    Yoğunluğu ve Ciro Payı Analizi
                  </h5>
                  <span className="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-md w-fit">
                    En Çok Kazandırandan Sıralı
                  </span>
                </div>

                {topServices.length === 0 ? (
                  <p className="text-xs text-slate-400 font-bold py-4">
                    Henüz tamamlanmış bir hizmet kaydı bulunamadı.
                  </p>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                    {topServices.map((service, index) => (
                      <div
                        key={index}
                        className="bg-slate-50/50 border border-slate-100 p-4 rounded-2xl flex flex-col justify-between gap-3"
                      >
                        <div className="flex justify-between items-start">
                          <div>
                            ...
                            <span className="text-[10px] text-slate-400 font-semibold">
                              {service.count} Randevuda Yapıldı
                            </span>
                          </div>
                        ...
                        <div className="flex justify-between items-center text-[11px] font-black text-slate-700">
                          ...
                          <span className="text-slate-900 font-extrabold">
                            ₺{service.totalRevenue.toLocaleString("tr-TR")}
                          </span>
                        </div>
                      </div>
                    ))}
            ...
            {(viewType === "all" || viewType === "employee") && (
              <div className="space-y-4 animate-fadeIn pt-4 border-t border-dashed border-slate-100">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <h5 className="text-xs sm:text-sm font-black text-slate-800 uppercase tracking-wider flex items-center gap-2">
                    <Users className="w-4 h-4 text-purple-500" /> Çalışan
                    (Personel) Performans ve Ciro Dağılımı
                  </h5>
                  <span className="text-[10px] font-bold text-purple-600 bg-purple-50 px-2 py-0.5 rounded-md w-fit">
                    Salona En Çok Ciro Sağlayandan Sıralı
                  </span>
                </div>

                {employeePerformance.length === 0 ? (
                  <p className="text-xs text-slate-400 font-bold py-4">
                    Henüz randevusu olan bir çalışan verisi gelmedi.
                  </p>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                    {employeePerformance.map((employee, index) => (
                      <div
                        key={index}
                        className="bg-slate-50/50 border border-slate-100 p-4 rounded-2xl flex items-center justify-between hover:bg-slate-50 transition-colors"
                      >
