...
export const EmployeesDetail: React.FC<DetailSectionProps> = ({ 
  ...
  const validateIntervals = (intervals: ShiftInterval[]): boolean => {
  ...
  const saveShiftsToDb = useCallback(async (dayKey: string, updatedDayObj: DayShiftState) => {
    if (updatedDayObj.active && !validateIntervals(updatedDayObj.intervals)) {
      setErrorMessages(prev => ({ ...prev, [dayKey]: "Geçersiz saat dizilimi veya çakışan oturum tespit edildi." }));
      return;
    }
    
    setErrorMessages(prev => ({ ...prev, [dayKey]: "" }));
  ...
  const addInterval = (dayKey: string) => {
    ...
    setShifts(prev => ({ ...prev, [dayKey]: updated }));
  ...
  const toggleServicePermissionInDb = async (serviceId: string) => {
    ...
    try {
      if (isAllowed) {
        ...
        setAllowedServiceIds(prev => prev.filter(id => id !== serviceId));
      } else {
        await supabase
          .from("employee_services")
          .insert({ employee_id: selectedStaff.id, service_id: Number(serviceId) });
        setAllowedServiceIds(prev => [...prev, serviceId]);
      }
    } catch (err) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="p-6 flex flex-col items-center text-center justify-center border border-slate-100 bg-white rounded-3xl">
          ...
          <div className="flex items-center gap-1.5 mt-2 text-xs font-semibold text-slate-500">
            ...
            <span>{selectedStaff.phone || "Telefon Belirtilmedi"}</span>
          </div>
      ...
      <Card className="p-6 border border-slate-100 bg-white rounded-3xl">
        ...
        <div className="space-y-4">
          {DAYS_OF_WEEK.map((day) => {
            ...
            const hasError = !!errorMessages[day.key];
            return (
              <div key={day.key} className="flex flex-col p-4 bg-slate-50 border border-slate-100 rounded-2xl gap-2 transition-all">
                ...
                {hasError && (
                  <p className="text-[11px] font-bold text-red-500 mt-1 pl-1">{errorMessages[day.key]}</p>
                )}
              </div>
            );
          })}
