...
import { EmployeesDetail } from "./pages/HairdresserPanel/Emloyees";
...
export default function App() {
  const [userRole, setUserRole] = useState<"hairdresser" | "admin" | "brand">(
    "brand" 
  );
  ...
  return (
    <div className="min-h-screen bg-brand-base text-slate-800 flex w-full overflow-x-hidden font-sans antialiased">
      ...
      <div className="flex-1 pl-72 flex flex-col min-h-screen w-full">
        {userRole === "admin" && <AdminHeader setUserRole={setUserRole} />}
        {userRole === "brand" && <BrandHeader setUserRole={setUserRole} />}
        {userRole === "hairdresser" && <HairdresserHeader setUserRole={setUserRole} />}
